...
### 1.1 Telegram 动态遮罩 (Spoilers)

使用 `||遮罩文本||` 语法包裹需要防剧透或保密的内容：

* **基础遮罩**：这是一段包含 ||绝密代码与通关秘籍|| 的文本，未点击时呈现赛博数据损坏与 CRT 微光噪点。
* **黑客解密动效**：点击上方遮罩，文字会经历赛博矩阵字符置换计算并恢复为真实内容；再次点击已揭示文本可重新折叠回损坏遮罩态。
* **MD3 动态色适配**：遮罩底色、扫描线与辉光自适应当前主题的 6 种动态主题色及深浅模式。
* **行内富文本嵌套**：遮罩内部支持富文本，例如 ||包含 **加粗重点** 与 [外部超链接](https://telegram.org) 的复合遮罩||。
* **代码块严格隔离**：代码块中的逻辑或按位或运算符保持安全原样，例如 `if (is_admin || is_owner)` 绝不会被误判。
