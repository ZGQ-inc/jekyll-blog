...
## 二、 扒皮 SpotX 所谓“修复”：自作聪明的骚操作与暗度陈仓

为了查明 SpotX 维护者到底写了什么神仙代码，我们第一时间检阅了 SpotX 在 2026 年 9 月 9 日发布的修复提交：[Commit 9fa954a](https://github.com/SpotX-Official/SpotX/commit/9fa954acd21d2f057d14244d128c9bde6aa01047){: .preview }。

翻开这个 Commit，其提交日志（Commit Message）赫然写着：
```text
version bump to 1.3.0

- fix the banner_home patch
- update css selectors for hiding download buttons, menu items, and quality settings
- update css selectors for static lyrics themes
- skip snapshot creation in spicetify #892
```

仔细审视 `run.ps1` 中的实际代码改动，在场的技术人员全被逗乐了：
...
```mermaid
flowchart LR
    A[SpotX Commit 9fa954a] --> B[骚操作: 大小写劫持 Go 语言前缀]
    B --> B1[利用 Spicetify 的 HasPrefix 区分大小写<br/>企图绕过 xpui-modules.js 快照解包]
    B1 --> B2[❌ 完全没解决 useNavigateStable 上下文问题]
    A --> C[暗度陈仓: 强升 Spotify 1.3.0]
    C --> C1[latest_full 从 1.2.99 升到 1.3.0<br/>引爆 Spicetify 业余版本比对 Bug]
    C1 --> C2[💥 触发非法语法注入，直接黑屏暴毙！]
```

### 1. 企图利用 Go 语言大小写敏感“偷鸡”，实则掩耳盗铃
为什么 SpotX 维护者要写这行 `Rename-Item` 把首字母 `v` 改成大写的 `V`？  
翻开 Spicetify 的 Go 源码（`preprocess.go:168`），你会发现 Spicetify 在检测快照时写的是：
```go
if !file.IsDir() && strings.HasPrefix(file.Name(), "v8_context_snapshot") && strings.HasSuffix(file.Name(), ".bin") { ... }
```
**Go 语言的 `strings.HasPrefix` 是严格区分大小写的！**  
SpotX 作者自以为发现了新大陆，心里盘算着：“只要我把 `v8_context_snapshot.bin` 重命名为大写的 `V8_context_snapshot.bin`，Spicetify 的 Go 扫描器就会匹配失败，从而跳过解包 `xpui-modules.js`，这样两边就不会冲突了！”

然而，这完全是**自作聪明、掩耳盗铃的“伪修复”**：
1. **根本没解决问题**：第一关中报错 `useNavigateStable must be used within a StableUseNavigateProvider` 的根因，是 Spicetify 扩展的调用脱离了 React Router 的 Context 树，**压根就不是因为快照有没有被解包**！即便跳过了快照解包，导航组件依然会因为脱离 Provider 而打崩整棵 React 树。
2. **治丝益棼**：更讽刺的是，Windows 系统的 CEF 内核本身读取文件是不区分大小写的，这个文件改了跟没改没有任何区别。

### 2. 暗度陈仓：顺手强推 1.3.0，引爆核弹级连环大碰撞！
如果说改大小写只是个逗人发笑的无效操作，那么该 Commit 里顺手带上的版本拉升，则是彻底将全体用户送上西天的核按钮：
```powershell
# run.ps1 顶部版本控制：
-   [string]$latest_full = "1.2.99",
+   [string]$latest_full = "1.3.0",
```
SpotX 维护者为了适配 Spotify 官方最新的广告规则和歌词 CSS 选择器，在完全没有对 Spicetify 联动进行任何测试的情况下，**瞒天过海、强行把用户底座升级到了全新的 1.3.0.277 大版本**！

正是这行顺手牵羊的升级，瞬间引爆了 Spicetify 内部潜伏已久的惊天大雷！
