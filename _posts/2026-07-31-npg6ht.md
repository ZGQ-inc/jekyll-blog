...
> [!TIP]
> **多边形与标记点 (Polygon & Point)**：
> 在同一个 GeoJSON 中，你可以同时混用多边形（如划定一个区域范围）和独立的坐标点。所有绘制的图形都会自动继承你当前主题的强调色，点击还可以查看它们携带的属性（Properties）。
  ...
  "features": [
    {
      ...
      "properties": {
        "name": "北京故宫区域范围 (Polygon)",
        "description": "这是一块自定义的多边形区域"
      }
    },
    {
      "type": "Feature",
...
<video src="https://r18.zgqinc.gq/rickroll.mp4" controls></video>

### 音乐播放器 (Audio)
简洁美观的内嵌式音频播放栏，支持丝滑的进度拖拽：

<audio src="https://zgqinc.gq/music/Running_in_the_Night-FM-84&Ollie_Wride.mp3" controls></audio>
